    ...
    if (memberName == "") {
        ...
        return (
            <div id='team-member' className="w-[100%] h-[10%] max-h-[8.5%] flex flex-row items-center mb-3">
                <p className="2xl:w-[16%] xl:w-[18%] lg:w-[35%] ml-4 font-bold 2xl:text-[1.30rem] xl:text-xl lg:text-lg md:text-md text-sm">Member: {memberName}</p>
    ...
    return(<>
        <div className="h-[85vh] bg-[#C7D1EB] flex items-center justify-center">
            <div id="" className="h-[60vh] w-[77vw] pl-8 pt-4 pb-4 pr-4 bg-white rounded-4xl flex flex-col shadow-xl align-middle">
                <h1 className="self-center relative font-bold text-lg md:text-4xl">{teamName}</h1>
                <p className="self-center relative font-bold text-md md:text-2xl">Access Code: {accessCode}</p>
                <div className="p-4 text-lg md:text-2xl text-[#83dac0] font-bold mb-3">Owner: {owner}</div>
                ...
                <button id="delete-btn" className={`md:w-[23%] w-[40%] h-[8%] bg-[#EA5945] hover:bg-[#df2d16] rounded-lg self-end mt-auto mb-4 mr-4 border border-black overflow-hidden text-white font-bold md:text-xl text-sm`} >{isOwner ? "Delete Team": "Leave Team"}</button>
            </div>
        </div>
    </>)
}
